...
class Users::PasswordResetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "accessing the password reset page with a valid token shows
the form" do
    @user.reset_password
    get edit_users_password_reset_path(@user.generate_token_for(:password_reset))

    assert_response :ok
    assert_select "form"
  end

  test "accessing the password reset page with an invalid or nil token redirects" do
    get edit_users_password_reset_path("invalid token")
